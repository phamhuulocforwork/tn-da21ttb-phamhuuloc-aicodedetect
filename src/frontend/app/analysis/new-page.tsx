...
import AnalysisSelector, { AnalysisMode } from "./_components/analysis-selector";
import ResultsDashboard from "./_components/results-dashboard";
import oneDarkPro from "./_components/onedarkpro.json";

import { apiClient, handleApiError, isAnalysisResponse } from "@/lib/api-client";
...
int main() {
  ...
  const [result, setResult] = useState<AnalysisResponse | IndividualAnalysisResponse | null>(null);
  ...
  const handleSubmitCode = useCallback(async (code: string, language: string): Promise<AnalysisResponse | IndividualAnalysisResponse> => {
    setIsAnalyzing(true);
    setError(null);
    ...
    try {
      ...
      if (isComprehensive) {
        toast.success("Analysis Complete", {
          description: `Overall score: ${Math.round(response.assessment.overall_score * 100)}% AI-like`
        });
      } else {
        toast.success("Analysis Complete", {
          description: `${Object.keys(response.features).length} features extracted`
        });
      }
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-muted-foreground">
              Analyze your code to detect AI-generated patterns and assess coding style characteristics.
            </p>
          </div>
            ...
            <div className="flex items-center justify-between">
              ...
              {code.trim() && (
                <div className="text-sm text-muted-foreground">
                  {code.split('\\n').length} lines • {Math.round(code.length / 1024 * 10) / 10} KB
                </div>
              )}
            </div>
